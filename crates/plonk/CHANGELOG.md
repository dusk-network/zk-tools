...
- Move circuit construction into the proof-system-neutral `dusk-zk-composer` crate.
  `dusk-plonk::prelude` continues to re-export the circuit API, while PLONK now
  derives its permutation wiring from the shared gate layout
  ([#5](https://github.com/dusk-network/zk-tools/issues/5)).
- Separate circuit-construction failures from proving-system failures. Circuit
  implementations now return `CircuitError`, and PLONK reports such failures
  as `Error::Circuit`.
